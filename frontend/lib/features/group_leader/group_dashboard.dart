...
final groupDashboardProvider =
    FutureProvider<GroupDashboardData>((ref) async {
  final dio = ref.read(dioProvider);
  final response = await dio.get(ApiEndpoints.groupDashboard);
  return GroupDashboardData.fromJson(response.data['data']);
});
...
class GroupDashboard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return ShellLayout(
      ...
      child: RefreshIndicator(
        onRefresh: () => ref.refresh(groupDashboardProvider.future),
        child: SingleChildScrollView(
          padding: const EdgeInsets.all(24),
          child: ref.watch(groupDashboardProvider).when(
                loading: () => Column(
                  children: [
                    GridView.count(
                      crossAxisCount: 2,
                ...
                error: (e, _) => ErrorState(
                  ...
                  onRetry: () => ref.invalidate(groupDashboardProvider),
                ),
  ...
  Widget _buildContent(BuildContext context, GroupDashboardData data) {
    return Column(
      ...
      children: [
        ...
        Row(
          ...
          children: [
            Expanded(
              flex: 3,
              child: _KpiSection(kpiProgress: data.kpiProgress),
            ),
            const SizedBox(width: 16),
            Expanded(
              flex: 2,
              child: _AttendanceSection(attendance: data.recentAttendance),
            ),
          ],
        ),
        const SizedBox(height: 24),
  ...
  Widget _buildStatsGrid(BuildContext context, GroupDashboardData data) {
        ...
        Wrap(
          ...
          children: [
            _QuickLink(
              label: 'View Members',
              icon: Icons.people_outline,
              onTap: () => context.go('/group/members'),
            ),
            _QuickLink(
              label: 'Mark Attendance',
              icon: Icons.how_to_reg_outlined,
              onTap: () => context.go('/group/attendance'),
            ),
          ],
        ),
      ],
...
class _KpiSection extends StatelessWidget {
  ...
  const _KpiSection({required this.kpiProgress});
